...
function formatShort(d: Date): string {
  return d.toLocaleDateString('uk-UA', { month: 'short', year: 'numeric', timeZone: 'UTC' });
}
...
function pct(date: Date, start: Date, end: Date): number {
  return Math.max(0, Math.min(100,
    ((date.getTime() - start.getTime()) / (end.getTime() - start.getTime())) * 100
  ));
}
...
function GanttView({ data }: { data: ChronologyData }) {
  ...
  // Year tick marks
  const ticks: Date[] = [];
  for (let y = rangeStart.getUTCFullYear() + 1; y <= rangeEnd.getUTCFullYear(); y++) {
  ...
  function showTip(e: React.MouseEvent, text: string) {
    ...
    if (!rect) return;
    setTooltip({ x: e.clientX - rect.left, y: e.clientY - rect.top, content: text });
  }
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', overflowX: 'auto' }}>
      ...
      >
        {/* Year tick lines */}
        {ticks.map(tick => {
          const x = LABEL_W + (pct(tick, rangeStart, rangeEnd) / 100) * (100 - (LABEL_W / 8));
          return (
            <g key={tick.getUTCFullYear()}>
              <line
                x1={`${x}%`} y1={0} x2={`${x}%`} y2={CHART_H - 24}
                stroke="#D7DEE5" strokeWidth={1} strokeDasharray="3 3"
              />
              <text
                x={`${x}%`} y={CHART_H - 8}
                textAnchor="middle" fontSize={10}
                fill="#5C6B7A" fontFamily="var(--mono)" letterSpacing={1}
              >
                {tick.getUTCFullYear()}
              </text>
            </g>
        ...
        {ships.map((ship, si) => {
          ...
          const barY = y + 14;
          const BAR_H = 16;
          const shipPositions = data.positions.filter(p => p.ship === ship.id);
          const shipEvents = data.events.filter(e => e.ships.includes(ship.id));

          return (
            <g key={ship.id}>
              {/* Row bg */}
              <rect x={0} y={y} width="100%" height={ROW_H - 4}
                fill={si % 2 === 0 ? '#F8F9FA' : '#FFFFFF'}
                opacity={0.5}
              />

              {/* Ship label */}
              <text x={8} y={barY + BAR_H / 2 + 4}
                fontSize={11} fontWeight={700} fill={ship.color}
              ...
              {shipPositions.map(pos => {
                ...
                const locName = data.meta.locations[pos.location]?.name ?? pos.location;
                return (
                  <rect
                    key={pos.id}
              ...
              {shipEvents.map(ev => {
                ...
                return (
                  <g key={ev.id} style={{ cursor: 'pointer' }}
                    onMouseMove={e => showTip(e, `${formatFull(ev.date)}\n${EVENT_LABELS[ev.type] ?? ev.type}\n${ev.title}`)}
                    onMouseLeave={() => setTooltip(null)}
                  >
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px 16px', marginTop: 12 }}>
        {Object.entries(EVENT_ICONS).map(([type, icon]) => (
          <span key={type} style={{
            ...
            fontFamily: 'var(--mono)', fontSize: 10, color: '#5C6B7A', letterSpacing: '0.05em',
          }}>
            <span style={{
              width: 10, height: 10, borderRadius: '50%',
              background: EVENT_COLORS[type], display: 'inline-block', flexShrink: 0,
            }} />
            {EVENT_LABELS[type]}
          </span>
        ))}
        <span style={{
          ...
          <span style={{ width: 24, height: 8, background: '#1F4E78', opacity: 0.3, display: 'inline-block', borderRadius: 2 }} />
          Базування
        </span>
      </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
        <button style={chipStyle(shipFilter === 'all', '#5C6B7A')} onClick={() => setShipFilter('all')}>
          Всі кораблі
        </button>
        {ships.map(ship => (
          <button key={ship.id} style={chipStyle(shipFilter === ship.id, ship.color)}
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 20 }}>
        <button style={chipStyle(typeFilter === 'all', '#5C6B7A')} onClick={() => setTypeFilter('all')}>
          Всі типи
        </button>
        {allTypes.map(type => (
          <button key={type} style={chipStyle(typeFilter === type, EVENT_COLORS[type] ?? '#5C6B7A')}
      ...
      {/* Event count */}
      <div style={{
        fontFamily: 'var(--mono)', fontSize: 10, color: '#5C6B7A',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
        {filtered.map(ev => {
          ...
          const eventShips = ev.ships.map(sid => ships.find(s => s.id === sid)).filter(Boolean) as ShipMeta[];
          ...
          return (
            <div key={ev.id} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 6 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{
                    ...
                    color: '#0E1B2A', lineHeight: 1.2, marginBottom: 4,
                  }}>
                    {ev.title}
                    {ev.verified === false && (
                      <span style={{
                        marginLeft: 6, fontSize: 9, fontFamily: 'var(--mono)',
                        color: '#8C2D04', letterSpacing: '0.1em', verticalAlign: 'middle',
                      }}>⚠ НЕПІДТВЕРДЖЕНО</span>
                    )}
                  </div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, alignItems: 'center' }}>
                    {eventShips.map(ship => (
                      <span key={ship.id} style={{
                        fontFamily: 'var(--mono)', fontSize: 9, letterSpacing: '0.08em',
                        color: ship.color, border: `1px solid ${ship.color}`,
                    ...
                    <span style={{
                      fontFamily: 'var(--mono)', fontSize: 9, color: '#5C6B7A',
                ...
                {/* Importance dots */}
                <div style={{ flexShrink: 0, display: 'flex', gap: 2, paddingTop: 3 }}>
              ...
              {ev.casualties && (
                <div style={{
                  fontFamily: 'var(--mono)', fontSize: 10, color: '#C8102E',
              ...
              {/* Description */}
              <div style={{
                fontSize: 13, lineHeight: 1.5, color: '#2C3E50',
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8, gap: 12 }}>
                <button
                  ...
                  style={{
                    ...
                    fontFamily: 'var(--mono)', fontSize: 10, color: '#1F4E78',
                ...
                {isOpen && ev.sources.length > 0 && (
                  <div style={{
                    fontFamily: 'var(--mono)', fontSize: 9, color: '#5C6B7A',
                    letterSpacing: '0.05em', textAlign: 'right', flex: 1,
                  }}>
                    {ev.sources.join(' · ')}
                  </div>
                )}
      ...
      {filtered.length === 0 && (
        <div style={{
          ...
          fontFamily: 'var(--mono)', fontSize: 11, color: '#5C6B7A',
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 0 }}>
          <button style={tabStyle(mode === 'gantt')} onClick={() => setMode('gantt')}>
            Хронологія
          </button>
          <button style={tabStyle(mode === 'journal')} onClick={() => setMode('journal')}>
            Журнал подій
          </button>
        </div>
      </div>
      ...
      <div style={{
        ...
        <span>
          {data.meta.ships.length} кораблів · {data.events.length} подій · v{data.meta.version}
        </span>
