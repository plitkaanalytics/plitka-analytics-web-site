...
// § 11. Квітневий конвой 2026 року: три порти виходу, спільний шлях у Тартус
// і повернення. Позначка судна їде цим шляхом під прокрутку, тому маршрут
// заданий суцільним — spb → med → back, без розривів на стиках.
const spbLeg = [
  [30.3, 59.93],
  [29.0, 60.0],
  [27.0, 59.9],
...
// Дві лінії підміни: від місця, де судно було насправді, до точки, яку
// показував AIS. Малюються пунктиром — це не маршрут, а брехня приладу.
const spoofEstLeg = [
  [0.0, 37.2],
  [24.7, 59.4],
];
...
// Дороги двох інших суден. «Спарта» виходить із Калінінграда й далі йде тим
// самим коридором, що й «Скобелев», тож її шлях — стик балтійського відрізка
// з хвостом петербурзького. «Пашин» іде мурманською дорогою, окремої не
// потребує. Назад конвой розходиться: «Спарта» в Калінінград (leg back),
// «Скобелев» у Петербург — це backSpb.
const joinAt = spbLeg.findIndex((p) => p[0] === 19.0 && p[1] === 56.0);
const spartaLeg = [...kgdLeg, ...spbLeg.slice(joinAt + 1)];
const backJoin = convoyBackLeg.findIndex((p) => p[0] === 19.0 && p[1] === 56.0);
const backSpbLeg = [
  ...convoyBackLeg.slice(0, backJoin + 1),
  ...spbLeg.slice(0, joinAt).reverse(),
];
// До Гібралтару обидва йшли разом; розходяться вони за протокою, тому дорога
// «Скобелева» додому починається саме звідти.
const gibBack = backSpbLeg.findIndex((p) => p[0] === -5.35 && p[1] === 36.0);
const skHomeLeg = backSpbLeg.slice(gibBack);

const AIS = {
  name: "ais",
  out: "public/articles/syriyskyi-ekspres/ais-convoy-map.json",
  baseBox: [
    [-16, 30],
    [40, 72],
  ],
  zones: [
    { box: [-18, 28, 42, 74], tol: 0.004 },
  ],
  tolFar: 0.25,
  tall: true,
  pad: 0.08,
  legs: {
    ...
    sparta: spartaLeg,
    skHome: skHomeLeg,
    spoofEst: spoofEstLeg,
    spoofKgd: spoofKgdLeg,
  },
};

const MAPS = [TARTUS, NORTH, AIS];
